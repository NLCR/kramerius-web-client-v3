...
describe('availableYears ownership', () => {
  ...
  function withYears(rootPid: string, years: any[]) {
    return periodicalDetailReducer(
      initialState,
      loadPeriodicalItemsSuccess({ parentVolumeUuid: VOLUME_A, children: [], availableYears: years, availableYearsRootPid: rootPid }),
    );
  }
  ...
  it('replaces the years when another periodical is loaded', () => {
    ...
    state = periodicalDetailReducer(
      state,
      loadPeriodicalItemsSuccess({ parentVolumeUuid: VOLUME_A, children: [], availableYears: yearsB, availableYearsRootPid: ROOT_B }),
    );
